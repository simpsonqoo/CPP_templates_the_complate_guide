...
但是如果要先宣告後之後才會實作，就會有些問題。而要實作operator <<，由於其不為function template，而是會和class一起被實體化的普通函式，所以必須在宣告前給定全部的型別修飾詞。
...
class Stack
{
    ...
    template <typename U>
    friend operator<< (std::ostream& os, Stack<U> const& s);
};
```